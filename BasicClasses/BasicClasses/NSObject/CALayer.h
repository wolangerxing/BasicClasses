...
// MARK: - 第 0 个讲解 CALayer, Core Animation 究竟是什么
/* Core Animation 它使帮助我们将图像读取成位图，通过硬件的处理，实现动画效果
 * CALayer 负责管理图像内容，并且允许你增加动画效果。
 * - 几何操作（例如rotate，transfrom）
 * - 内容（image等）
 * - 可视属性（backgroundColor，alpha）*/
...
// MARK: - 第 1 个讲解 CALayer 可以通用
/* 注意 CALayer 其实可以通用
 * 包括 macOS、watchOS 以及 tvOS
 * 一定程度上解释了为什么有了 UIView 还需要 CALayer */
...
/* Core Animation to create shadow copies of layers
 * e.g. for use as presentation layers. Subclasses can override
...
// MARK: - 第 2 个讲解 CALayer 的三层树结构
/* 注意 CALayer 一共有三层树结构
 * Render Tree 为 CoreAnimation 私有的，是 CoreAnimation 具体实现使用的私有 Tree
 * Model Layer Tree：我们通常处理的对象，比如 layer.position = CGPoint(.zero) 修改的就是 Model Layer Tree
 ...
 * Core Animation可以通过geometryFlipped属性来适配这两种情况，它决定了一个图层的坐标是否相对于父图层垂直翻转
 * 在 iOS 上通过设置它为 true 意味着它的子图层将会被垂直翻转
 */
...
// MARK: - 第 6 个讲解 触摸和手势事件 containsPoint、hitTest
 ...
 * zPosition 属性可以明显改变屏幕上图层的顺序，但不能改变事件传递的顺序
 */
...
// MARK: - 第 7 个讲解 contents 相关的属性
/* contentsRect 在图层的上显示寄宿图的摸个部分，也就是截取寄宿图的某个部分来填充满整个图层
 * contentsGravity 类似UIImageView的contentMode属性,是一个NSString类型
 * contentsScale 就是表示一个点表示几个像素
 */
...
// MARK: - 第 8 个讲解 拉伸过滤
/* 作用于原图的像素上并根据需要生成新的像素显示在屏幕上
 * CALayer为此提供了三种拉伸过滤方法，kCAFilterLinear、kCAFilterNearest、kCAFilterTrilinear
...
// MARK: - 第 9 个讲解 组透明
/* 解决透明度的混合叠加的问题
 * 你也可以通过设置 Info.plist 文件中的 UIViewGroupOpacity 为 YES 来达到这个效果，但是这个设置会影响到这个应用
 */
